...
#
# ------------------------------------------------------------------------------------
# blog-incidencia0620covid/clean-data/src/clean.R
...
files <- list(input_idenm = here("import/output/idenm.rds"),
              input_pob_est = here("import/output/pob-estatal.rds"),
              output_ide = here("clean-data/output/ide.rds")
              )

# Abrimos archivos
bases <- list(df_ide = readRDS(files$input_idenm), 
...
ide <- filter(bases$df_ide, str_detect(subtipo_de_delito, 
  ...
  mutate(total_anual = rowSums(select(., enero:diciembre), na.rm = T),
         ...
         subtipo_de_delito = case_when(str_detect(subtipo_de_delito, "Violación") ~ "Violación",
                                       str_detect(subtipo_de_delito, "Acoso|Hostigamiento") ~ "Hostigamiento o acoso sexual",
                                       T ~ subtipo_de_delito)) %>% 
  ...
  select(year:tipo_delito, total_enero:total_junio, total_enero_junio, total_anual)

# 1.2: Reshapeamos para sumar homicidios con feminicidios
ide <- pivot_longer(ide, total_enero:total_anual, names_to = "tipo_total", values_to = "valor") %>% 
...
ide <- left_join(ide, pob_est, by = c("year" = "ano", "cve_ent" = "cve_geo")) %>% 
  mutate(nom_ent = case_when(str_detect(nom_ent, "Coahuila") == T ~ "Coahuila",
